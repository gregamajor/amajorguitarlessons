$(document).ready(function(){
    ...
    $('#footer-submit').click(function(e){
        ...
        if(email.length == 0 || email.indexOf('@') == '-1'){
            var error = true;
            $('#footer-form-email').parent('div').addClass('field-error');
        }else{
            $('#footer-form-email').parent('div').removeClass('field-error');
        }
        ...
        if(error == false){
            $.post("submit.php", $("#footer-subscribe").serialize(),function(result){
                if(result == 'sent'){
                    $('#success-notification').addClass('show-up');
                    $('footer-submit').addClass('disabled');
                }
            });
        }
    });
